...
def run_scan(target):
    try:
        ...
        ip = safe_exec(lambda: socket.gethostbyname(target), "N/A")
        socketio.emit("info", {"ip": ip})

        log("Nmap...")
        ports = safe_exec(lambda: run_nmap(target), [])
        socketio.emit("ports", ports)

        log("Headers...")
        hdr = safe_exec(lambda: headers(target), {})
        tech = fingerprint(hdr)
        socketio.emit("tech", tech)

        log("Nuclei...")
        vulns = safe_exec(lambda: run_nuclei(target), [])
        socketio.emit("vulns", vulns)

        log("Fuzzing...")
        raw = safe_exec(lambda: run_ffuf(target), [])
        ...
        try:
            ...
            log("PDF generado", "success")
        except Exception as e:
            log(f"PDF error: {str(e)}", "error")

        log("Scan completado", "success")
